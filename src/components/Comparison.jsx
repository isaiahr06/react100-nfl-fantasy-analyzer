...
// --------------------
  ...
  function getPositionGroup(position) {
    ...
    if (
      ["Running Back", "Fullback"].includes(position)
    ) {
      return "RB";
    }

    if (
      ["Wide Receiver", "Tight End"].includes(position)
    ) {
      return "RECEIVER";
    }
  ...
  const player1Position = getPositionGroup(
    player?.position?.name
  );

  const player2Position = getPositionGroup(
    player2?.position?.name
  );
